...
void algorithms::balas_1959(QTextEdit *qt)
{
...
void algorithms::backtrack(QList<int> *selected, const int vars){
...
bool algorithms::backtrack_condition(QList<int> *selected,QList<int> *violated,QList<int> *helpfull,QList<QList<int>> *helpfull_per_constraint,QList<int> *preference , int const vars){
...
bool algorithms::termination_condition(QList<int> *selected, int const vars1){
    if(selected->size()==0){
        return false;
    }
    for(int i=0;i<selected->size();i++){
